...
public class SingleChatMessageHandler extends AbstractMessageHandler {
    ...
    protected void handleNewMessageReceive(AppMessage appMessage) {
        Logger.d("action: 收到单聊消息：" + appMessage);
        int contentType = appMessage.getHead().getContentType();
        switch (contentType) {
            case 0:
                Logger.d("action: 撤销单聊消息");
                break;
            case 1:
                Logger.d("action: 文本单聊消息");
                break;
            case 2:
                Logger.d("action:图片单聊消息");
                break;
            case 3:
                Logger.d("action: 音频单聊消息");
                break;
            case 4:
                Logger.d("action: 视频单聊消息");
                break;
        }
    ...
    protected void handleMessageStatusChange(AppMessage appMessage, int status) {
        Logger.d("handleMessageStatusChange: 更新单聊消息状态： status：" + status);
